...
const UpdatePrj = ({ setUpD, prjId }) => {

    const { loaded, updatePrj } = useApp()
    ...
    return (
        <div className='w-[30vw] bg-[#3C3D37] z-50 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-10 py-6 rounded-xl'>
            <div className='flex justify-between'>
                <div className='font-semibold text-xl text-emerald-500'>
                    Editing Section
                </div>
                <div onClick={() => setUpD("notActive")} className='cursor-pointer font-thin tracking-widest text-xl text-red-500'>
                    ❌
                </div>
            </div>
            <form onSubmit={onSubmit} className='px-20 py-12 flex justify-center items-center flex-col'>
                ...
                <button type='submit' className='cursor-pointer bg-[#697565] px-8 py-3 rounded-2xl text-xl'>Update Project</button>
            </form>
        </div>
    )
}

export default UpdatePrj
